...
class Exurcion{
  ...
  getListExurcion(int id) async {
    var request = http.MultipartRequest('POST', Uri.parse('${url}polls/GetListExurcion'));
    request.fields.addAll({
    ...
    if (response.statusCode == 200) {
      ...
      if (body01["Response"] == "Success") {
        ...
        String fields =json.encode(body02[0]["fields"]);
        int id=body02[0]["pk"];
        this.listExurcion = await ListExurcion(fields: fields,id:id);
        total_prix=total_prix+listExurcion.prix;
        await Future.delayed(const Duration(milliseconds: 500));
      }
    }
    else {}
  ...
  Exurcion({int id=0,String fields=""}){
    if(fields!=""){
      ...
      getListExurcion(content["id_ListExurcion"]);
    }
