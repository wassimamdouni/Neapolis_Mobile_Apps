// ignore_for_file: camel_case_types, use_key_in_widget_constructors, non_constant_identifier_names, prefer_final_fields, await_only_futures, deprecated_member_use, must_be_immutable, no_logic_in_create_state, avoid_unnecessary_containers
...
class VoitureUpdate_Page extends StatefulWidget {

  Voiture voiture;
  VoitureUpdate_Page({required this.voiture});

  @override
  State<VoitureUpdate_Page> createState() => _VoitureUpdate_PageState(voiture: voiture);

}

class _VoitureUpdate_PageState extends State<VoitureUpdate_Page> {

  Voiture voiture;


  _VoitureUpdate_PageState({required this.voiture});
  ...
  List<Options> listOptiondelete=[];
  List<Options> listOptionadd=[];
      ...
      if(body01["Response"]=="Success") {
        ...
        for(int i =0;i<body02.length;i++){
          ...
          setState(() {
            ...
            if(id==voiture.marquer!.id){
              indexMarqueSelected=i;
            }
          });
          await Future.delayed(const Duration(milliseconds: 500));
        }
  ...
  updateVoiture() async {
    var request = http.MultipartRequest('POST', Uri.parse('${url}polls/UpdateVoiture'));
    request.fields.addAll({
      'id':voiture.numero_series,
    ...
    if (response.statusCode == 200) {
      ...
      if (body01["Response"] == "Success") {
        String id=numero_series_Controller.text;
        await Future.delayed(const Duration(seconds: 1));

        for(int i=0;i<listOptionadd.length;i++){
          await inserOption(listOptionadd[i].title, listOptionadd[i].descriptions, id);
          await Future.delayed(const Duration(seconds: 1));
        }
        await Future.delayed(const Duration(seconds: 1));

        for(int i=0;i<listOptiondelete.length;i++){
          await deleteOption(listOptiondelete[i].id);
          await Future.delayed(const Duration(milliseconds: 500));
        }
        await Future.delayed(const Duration(milliseconds: 500));
      }
  ...
  inserOption(String title,String descriptions,String numero_series) async {
  ...
  deleteOption(int id) async {
    var request = http.MultipartRequest('POST', Uri.parse('${url}polls/DeleteOption'));
    request.fields.addAll({
      'id': id.toString()
    });
    http.StreamedResponse response = await request.send();
  ...
  void initState() {
    ...
    getVoiture();
    getAllMarquers();
    numero_series_Controller.text = voiture.numero_series;
    modele_Controller.text  = voiture.modele;
    class_voiture_Controller.text  = voiture.class_voiture;
    annee_Controller.text  = voiture.annee;

    boite_Controller.text  = voiture.boite;
    nb_seats_Controller.text  = voiture.nb_seats.toString();
    nb_bags_Controller.text  = voiture.nb_bags.toString();
    nb_ports_Controller.text  = voiture.nb_ports.toString();


    etat_Controller.text  = voiture.etat;
    description_Controller.text  = voiture.description;

    caution_Controller.text  = voiture.caution.toString();
    prix_jour_Controller.text  = voiture.prix_jour.toString();

    color=voiture.color;

    image_voiture=File(voiture.photo);

    listOption = voiture.listoptions;
    result_total_Option = voiture.listoptions.length;
              ...
              if(bodyValidation){
                showDialog<String>(
                  ...
                  builder: (BuildContext context) => AlertDialog(
                    ...
                    content: Text('Êtes-vous sûr de vouloir vous Enregistre les modifications de la voiture ?',style: Theme.of(context).textTheme.caption!.apply(color:Colors.grey)),
                              ...
                              children:List<Widget>.generate(result_total_Option+1,(index) {
                                if(index==result_total_Option){
                                  ...
                                  return Chip(
                                    onDeleted: () {
                                      setState(() {
                                        if(listOption[index].id!=0){
                                          listOptiondelete.add(listOption[index]);
                                        }
                                        if(listOption[index].id==0){
                                          listOptionadd.remove(listOption[index]);
                                        }
                                        listOption.remove(listOption[index]);
                                        result_total_Option--;
                                      });
                                    },
                                    label:Tooltip(
